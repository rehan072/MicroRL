...
class Maze(MiniWorldEnv, utils.EzPickle):
    ...
    def _gen_world(self):
        ...
        visit(0, 0)
        edge = self.num_cols*self.room_size + 16*self.gap_size      
        center = edge/2

        self.box = self.place_entity(Box(color="red", size=1), pos=np.array([center,0,center]))

        self.place_agent(pos = np.array([(self.room_size/2 + self.gap_size), 0, (edge-(self.room_size/2 + self.gap_size))]), dir= math.pi/2)
